...
function BookingContent() {
  ...
  useEffect(() => {
    // 1. Try from query parameter ?id=...
    let id = searchParams.get('id');
    
    // 2. If not in query, try from window.location.pathname (/booking/PTN-...)
    ...
    if (id) {
      ...
      setLoading(false);
      setError('ไม่พบรหัสการจอง กรุณาระบุรหัสการจองคิว');
    }
  }, [searchParams]);

  const fetchBooking = async (id: string, showLoading = true) => {
    if (!id) return;
    ...
    try {
      ...
      if (prevStatusRef.current && prevStatusRef.current !== fetchedBooking.status) {
        ...
        sendQueueNotification({
          ...
          url: `/booking?id=${fetchedBooking.booking_id}`,
        });
      }
  ...
  const handleShareLine = () => {
    if (!booking) return;
    const url = typeof window !== 'undefined' ? window.location.href : `https://ptn-queue-booking.pages.dev/booking?id=${booking.booking_id}`;
...
${url}`;
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      console.warn('html-to-image error, using canvas fallback:', e);
    }
      ...
      <div className="py-6 sm:py-8 px-4 sm:px-6 lg:px-8 pb-24 sm:pb-12">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => fetchBooking(bookingId, false)}
              disabled={refreshing}
        ...
        <div className="ticket-card bg-white rounded-3xl border border-slate-200/80 shadow-xl overflow-hidden">
          <div className={`bg-gradient-to-r ${statusInfo.headerBg} p-6 sm:p-8 text-white text-center relative`}>
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
              <div className="inline-block bg-white/15 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/30 text-sm sm:text-base font-mono font-bold tracking-wider">
                {booking.booking_id}
              </div>
              <div className="inline-flex items-center gap-1.5 bg-amber-400 text-slate-950 px-4 py-1.5 rounded-full font-black text-sm sm:text-base shadow-md">
          ...
          <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="flex-1">
                ...
                {booking.admin_reason && (
                  <div className="mt-2.5 p-3 bg-white/80 rounded-xl border border-rose-200 text-xs text-rose-900 font-medium">
                    <span className="font-bold">บันทึกเหตุผล:</span> {booking.admin_reason}
                  </div>
                )}
              </div>
            </div>

            {/* 🔔 Web Push Notification & Browser Alert Card */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 no-print">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 py-3 text-white rounded-2xl font-bold text-xs sm:text-sm shadow-md transition active:scale-[0.98] ${
                  lineCopiedToast
            ...
            {lineCopiedToast && (
              <div className="p-3 bg-emerald-50 border border-emerald-300 rounded-2xl text-emerald-900 text-xs sm:text-sm font-semibold flex items-center gap-2.5 animate-in fade-in slide-in-from-top-1 shadow-sm">
